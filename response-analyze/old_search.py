    ...
    for row in rows:
        ...
        try:
            ...
            if flag:
                ...
                old_fail = old_fail + 1
            print("旧接口==>", old_totle, "==>", flag, "/", arr[0], "/", arr[1])
        except:
            print("旧接口==>", old_totle, "==>error==>", "error", row)
            old_err = old_err + 1
...
def old_project_publicity_search(rows):
    ...
    for row in rows:
        ...
        try:
            ...
            old_policy_request_template = {
                "conditions": {
                    "policyType": "merge_public_document"
                },
                "current": 1,
                "fromSite": "sub",
                "pageSize": 3,
...
def old_notice_search(rows):
    ...
    for row in rows:
        ...
        try:
            ...
            old_policy_request_template = {
                "conditions": {
                    "policyType": "merge_policy_notification"
                },
                "current": 1,
                "fromSite": "sub",
                "pageSize": 3,
    ...
    for row in rows:
        ...
        try:
            ...
            for doc in docs:
                ...
                type = doc['type']
                recdata = doc['data']
                if type == 'library_project':
                    proid_doc = recdata['projectName']
                    if proid == proid_doc:
                        flag = True
                        break
                elif type == 'subsidy_document':
                    proid_doc = recdata['project_id']
                    if proid == str(proid_doc):
                        flag = True
                        break
                else:
                    proid_doc = recdata['id']
                    if proid == str(proid_doc):
                        flag = True
                        break
            if flag:
